...
# Import DFT code interfaces
from multi_agent_dft.dft import (
    GaussianInterface,
    VASPInterface,
    CP2KInterface,
    Psi4Interface,
...
class MultiAgentSystem:
    ...
    def run(self):
        ...
        print("2. Input generation (CP2K/VASP/Gaussian/Psi4/ORCA/CRYSTAL)")
        print("3. Output processing (extract forces, energies, coordinates)")
        print("4. ML potential dataset creation (JSON or XYZ to MACE HDF5)")
        print("5. AIMD processing (JSON to CP2K/VASP AIMD inputs)")
        print("6. VASP MD input generation")
...
####### INPUT GENERATION METHODS ##########
    ...
    def _handle_simulation_input_generation(self, use_supercell=False, supercell_dims=None):
        """Guided DFT code input generation handlers."""
        print("\nAvailable DFT codes:")
        print("1. CP2K")
        print("2. VASP")
        print("3. Gaussian")
        ...
        if code == "cp2k":
            ...
            self._handle_psi4_input_generation(use_supercell=use_supercell, supercell_dims=supercell_dims)
        elif code == "orca":
            self._handle_orca_input_generation(use_supercell=use_supercell, supercell_dims=supercell_dims)
        elif code == "crystal":
            self._handle_crystal_input_generation(use_supercell=use_supercell, supercell_dims=supercell_dims)
        else:
...
####### PSI4 INPUT GENERATION METHODS ##########

    def _handle_psi4_input_generation(self, use_supercell=False, supercell_dims=None):
        """Handle Psi4 input generation with detailed step-by-step guidance."""
        print("\n==== Psi4 Input File Generation (Guided Mode) ====\n")
        ...
        output_dir = input("Enter output directory for Psi4 input files [psi4_inputs]: ").strip() or "psi4_inputs"
        output_path = Path(output_dir).expanduser().resolve()
        output_path.mkdir(parents=True, exist_ok=True)

        # Step 3: Get Psi4 parameters
        print("\n--- Psi4 Parameters ---")
        config = {}

        # Method
        print("\nSelect method:")
        print("1. B3LYP (Hybrid DFT)")
        print("2. PBE (Pure DFT)")
        print("3. MP2 (Post-HF)")
        print("4. CCSD(T) (Coupled Cluster)")
        print("5. Custom")
        method_choice = input("Choice [1]: ").strip() or "1"
        method_map = {"1": "B3LYP", "2": "PBE", "3": "MP2", "4": "CCSD(T)"}
        if method_choice in method_map:
            config['method'] = method_map[method_choice]
        else:
            config['method'] = input("Enter method: ").strip()

        # Basis set
        print("\nSelect basis set:")
        print("1. def2-TZVP (Balanced)")
        print("2. def2-SVP (Fast)")
        print("3. def2-QZVP (Accurate)")
        print("4. aug-cc-pVTZ")
        print("5. Custom")
        basis_choice = input("Choice [1]: ").strip() or "1"
        basis_map = {"1": "def2-TZVP", "2": "def2-SVP", "3": "def2-QZVP", "4": "aug-cc-pVTZ"}
        if basis_choice in basis_map:
            config['basis_set'] = basis_map[basis_choice]
        else:
            config['basis_set'] = input("Enter basis set: ").strip()

        # Job type
        print("\nSelect job type:")
        print("1. Optimize geometry")
        print("2. Single point energy")
        print("3. Frequency calculation")
        print("4. Gradient calculation")
        job_choice = input("Choice [1]: ").strip() or "1"
        job_map = {"1": "optimize", "2": "energy", "3": "frequency", "4": "gradient"}
        config['job_type'] = job_map.get(job_choice, "optimize")

        # Charge and multiplicity
        config['charge'] = int(input("\nCharge [0]: ").strip() or "0")
        config['multiplicity'] = int(input("Multiplicity [1]: ").strip() or "1")

        # Memory and threads
        config['memory'] = input("Memory [4 GB]: ").strip() or "4 GB"
        config['nthreads'] = int(input("Number of threads [4]: ").strip() or "4")

        # Create Psi4Interface
        psi4_interface = Psi4Interface(config)

        # Process files
        from multi_agent_dft.file_processing.cif import cif_to_xyz

        if input_path.is_file():
            # Single file processing
            self._process_single_psi4_file(input_path, output_path, psi4_interface, use_supercell, supercell_dims)
        else:
            ...
            if cif_files:
                print(f"\nFound {len(cif_files)} CIF files. Converting to XYZ...")
                for cif_file in cif_files:
                    xyz_output = input_path / f"{cif_file.stem}.xyz"
                    cif_to_xyz(str(cif_file), str(xyz_output))
                xyz_files = list(input_path.glob("*.xyz"))

            print(f"\nProcessing {len(xyz_files)} XYZ files...")
            for xyz_file in xyz_files:
                self._process_single_psi4_file(xyz_file, output_path, psi4_interface, use_supercell, supercell_dims)

        print(f"\nPsi4 input generation complete. Files saved to {output_path}")

    def _process_single_psi4_file(self, file_path, output_dir, psi4_interface, use_supercell, supercell_dims):
        """Process a single file for Psi4 input generation."""
        from multi_agent_dft.utils.validator import validate_structure
        from multi_agent_dft.utils.converter import convert_input_format

        try:
            # Read and validate structure
            structure_data = convert_input_format(str(file_path))
            validate_structure(structure_data)

            # Generate output filename
            output_file = output_dir / f"{file_path.stem}.in"

            # Generate input
            psi4_interface.generate_input(structure_data, output_file)
            print(f"  Generated: {output_file.name}")

        except Exception as e:
            print(f"  Error processing {file_path.name}: {e}")

####### ORCA INPUT GENERATION METHODS ##########

    def _handle_orca_input_generation(self, use_supercell=False, supercell_dims=None):
        """Handle ORCA input generation with detailed step-by-step guidance."""
        print("\n==== ORCA Input File Generation (Guided Mode) ====\n")
        ...
        # Step 2: Get output directory
        output_dir = input("Enter output directory for ORCA input files [orca_inputs]: ").strip() or "orca_inputs"
        output_path = Path(output_dir).expanduser().resolve()
        output_path.mkdir(parents=True, exist_ok=True)

        # Step 3: Get ORCA parameters
        print("\n--- ORCA Parameters ---")
        config = {}
        ...
        print("1. B3LYP (Hybrid DFT)")
        print("2. PBE (GGA DFT)")
        print("3. PBE0 (Hybrid GGA)")
        print("4. wB97X-D3 (Range-separated with dispersion)")
        print("5. Custom")
        method_choice = input("Choice [1]: ").strip() or "1"
        method_map = {"1": "B3LYP", "2": "PBE", "3": "PBE0", "4": "wB97X-D3"}
        if method_choice in method_map:
            config['method'] = method_map[method_choice]
        else:
            config['method'] = input("Enter method: ").strip()
        ...
        print("2. def2-SVP (Fast)")
        print("3. def2-QZVP (Accurate)")
        print("4. ma-def2-TZVP (Minimally augmented)")
        print("5. Custom")
        basis_choice = input("Choice [1]: ").strip() or "1"
        basis_map = {"1": "def2-TZVP", "2": "def2-SVP", "3": "def2-QZVP", "4": "ma-def2-TZVP"}
        if basis_choice in basis_map:
            config['basis_set'] = basis_map[basis_choice]
        else:
            config['basis_set'] = input("Enter basis set: ").strip()
        ...
        print("3. Frequency calculation")
        job_choice = input("Choice [1]: ").strip() or "1"
        job_map = {"1": "Opt", "2": "SP", "3": "Freq"}
        config['job_type'] = job_map.get(job_choice, "Opt")
        ...
        print("3. D3BJ")
        print("4. D4")
        disp_choice = input("Choice [3]: ").strip() or "3"
        disp_map = {"2": "D3", "3": "D3BJ", "4": "D4"}
        if disp_choice in disp_map:
            config['dispersion'] = disp_map[disp_choice]
        ...
        config['nprocs'] = int(input("Number of processors [4]: ").strip() or "4")
        config['maxcore'] = int(input("Memory per core in MB [2000]: ").strip() or "2000")

        # Create ORCAInterface
        orca_interface = ORCAInterface(config)
        ...
        if input_path.is_file():
            # Single file processing
            self._process_single_orca_file(input_path, output_path, orca_interface, use_supercell, supercell_dims)
        else:
            # Directory processing
            xyz_files = list(input_path.glob("*.xyz"))
            cif_files = list(input_path.glob("*.cif"))
            ...
            print(f"\nProcessing {len(xyz_files)} XYZ files...")
            for xyz_file in xyz_files:
                self._process_single_orca_file(xyz_file, output_path, orca_interface, use_supercell, supercell_dims)

        print(f"\nORCA input generation complete. Files saved to {output_path}")

    def _process_single_orca_file(self, file_path, output_dir, orca_interface, use_supercell, supercell_dims):
        """Process a single file for ORCA input generation."""
        from multi_agent_dft.utils.validator import validate_structure
        from multi_agent_dft.utils.converter import convert_input_format
        ...
        try:
            ...
            # Generate input
            orca_interface.generate_input(structure_data, output_file)
            print(f"  Generated: {output_file.name}")

        except Exception as e:
            print(f"  Error processing {file_path.name}: {e}")

####### CRYSTAL INPUT GENERATION METHODS ##########

    def _handle_crystal_input_generation(self, use_supercell=False, supercell_dims=None):
        """Handle CRYSTAL input generation with detailed step-by-step guidance."""
        print("\n==== CRYSTAL Input File Generation (Guided Mode) ====\n")
        print("Note: CRYSTAL is designed for periodic systems (crystals, slabs, polymers)")
        ...
        # Step 2: Get output directory
        output_dir = input("Enter output directory for CRYSTAL input files [crystal_inputs]: ").strip() or "crystal_inputs"
        output_path = Path(output_dir).expanduser().resolve()
        output_path.mkdir(parents=True, exist_ok=True)

        # Step 3: Get CRYSTAL parameters
        print("\n--- CRYSTAL Parameters ---")
        config = {}
        ...
        print("0. Molecule (0D)")
        print("1. Polymer (1D)")
        print("2. Slab/Surface (2D)")
        print("3. Crystal (3D)")
        dim_choice = input("Choice [3]: ").strip() or "3"
        config['dimensionality'] = int(dim_choice)

        # Hamiltonian
        print("\nSelect Hamiltonian:")
        print("1. DFT")
        print("2. Hartree-Fock")
        ham_choice = input("Choice [1]: ").strip() or "1"
        config['hamiltonian'] = "DFT" if ham_choice == "1" else "HF"

        # Functional (if DFT)
        if config['hamiltonian'] == "DFT":
            print("\nSelect functional:")
            print("1. PBE")
            print("2. B3LYP")
            print("3. HSE06")
            print("4. Custom")
            func_choice = input("Choice [1]: ").strip() or "1"
            func_map = {"1": "PBE", "2": "B3LYP", "3": "HSE06"}
            if func_choice in func_map:
                config['functional'] = func_map[func_choice]
            else:
                config['functional'] = input("Enter functional: ").strip()

        # Basis set
        print("\nSelect basis set:")
        print("1. pob-TZVP (Default)")
        print("2. pob-DZVP")
        print("3. STO-3G")
        print("4. 6-31G")
        print("5. Custom")
        basis_choice = input("Choice [1]: ").strip() or "1"
        basis_map = {"1": "pob-TZVP", "2": "pob-DZVP", "3": "sto-3g", "4": "6-31g"}
        if basis_choice in basis_map:
            config['basis_set'] = basis_map[basis_choice]
        else:
            config['basis_set'] = input("Enter basis set: ").strip()
        ...
        # K-point mesh
        print("\nK-point mesh (shrinking factors):")
        shrink1 = int(input("Shrinking factor 1 [8]: ").strip() or "8")
        shrink2 = int(input("Shrinking factor 2 [8]: ").strip() or "8")
        config['shrink'] = [shrink1, shrink2]

        # Geometry optimization
        opt_choice = input("\nOptimize geometry? (y/n) [n]: ").strip().lower()
        config['optimize_geometry'] = (opt_choice == 'y')

        # Create CRYSTALInterface
        crystal_interface = CRYSTALInterface(config)
        ...
        if input_path.is_file():
            # Single file processing
            self._process_single_crystal_file(input_path, output_path, crystal_interface, use_supercell, supercell_dims)
        else:
            # Directory processing
            xyz_files = list(input_path.glob("*.xyz"))
            cif_files = list(input_path.glob("*.cif"))
            ...
            print(f"\nProcessing {len(xyz_files)} XYZ files...")
            for xyz_file in xyz_files:
                self._process_single_crystal_file(xyz_file, output_path, crystal_interface, use_supercell, supercell_dims)

        print(f"\nCRYSTAL input generation complete. Files saved to {output_path}")

    def _process_single_crystal_file(self, file_path, output_dir, crystal_interface, use_supercell, supercell_dims):
        """Process a single file for CRYSTAL input generation."""
        from multi_agent_dft.utils.validator import validate_structure
        from multi_agent_dft.utils.converter import convert_input_format
        ...
        try:
            ...
            # Generate output filename
            output_file = output_dir / f"{file_path.stem}.d12"

            # Generate input
            crystal_interface.generate_input(structure_data, output_file)
            print(f"  Generated: {output_file.name}")
...
#### ML DATASET CREATION METHODS ######
    ...
    def _handle_ml_dataset_creation(self):
        """Handle creation of machine learning potential datasets from JSON/XYZ files or directories."""
        print("\n" + "="*80)
        print("ML POTENTIAL DATASET CREATION".center(80))
        print("="*80)
        
        print("\nSupported input formats:")
        print("  • JSON files (AMLP format with dict-based coordinates)")
        print("  • Extended XYZ files (ASE-compatible with Lattice, Properties, energy, forces)")
        print("  • Directories containing JSON and/or XYZ files")
        print("  • Multiple files/directories (comma or space separated)")
        print("  • File list (text file with paths, one per line)")
        
        # Step 1: Get input source type
        print("\n" + "-"*80)
        print("INPUT SOURCE".center(80))
        print("-"*80)
        print("\nHow would you like to provide input data?")
        print("  1. Single file (JSON or XYZ)")
        print("  2. Multiple files (comma or space separated)")
        print("  3. Directory (all JSON/XYZ files will be loaded)")
        print("  4. Multiple directories")
        print("  5. File list (text file containing paths)")
        
        input_mode = input("\nSelect input mode (1-5) [1]: ").strip() or "1"
        ...
        if input_mode == "1":
            # Single file
            file_path = input("\nFull path to data file (JSON or XYZ): ").strip()
            if not file_path:
            ...
            if not file_path.exists():
                ...
                return
            if file_path.is_dir():
                print(f"Error: {file_path} is a directory. Use option 3 for directories.")
                return
            ...
            # Multiple files
            files_input = input("\nEnter file paths (comma or space separated): ").strip()
            if not files_input:
            ...
            import re
            raw_paths = re.split(r'[,\s]+', files_input)
            ...
            for p in raw_paths:
                ...
                if not path.exists():
                    ...
                    continue
                if path.is_dir():
                    print(f"Warning: {path} is a directory, skipping. Use option 4 for directories.")
                    continue
            ...
            print(f"\n  ✓ Found {len(input_paths)} valid files")
        
        elif input_mode == "3":
            # Single directory
            dir_path = input("\nPath to directory containing data files: ").strip()
            if not dir_path:
            ...
            if not dir_path.is_dir():
                print(f"Error: {dir_path} is not a directory. Use option 1 for single files.")
                return
            
            input_paths = [str(dir_path)]
            
            # Ask about recursive search
            recursive_input = input("Search subdirectories recursively? (y/n) [n]: ").strip().lower()
            recursive = recursive_input == 'y'
        
        elif input_mode == "4":
            # Multiple directories
            dirs_input = input("\nEnter directory paths (comma or space separated): ").strip()
            if not dirs_input:
                print("Error: No directory paths provided.")
                return
            
            import re
            raw_paths = re.split(r'[,\s]+', dirs_input)
            ...
            print(f"\n  ✓ Found {len(input_paths)} valid directories")
            ...
            # File list
            list_path = input("\nPath to file list (text file with paths): ").strip()
            if not list_path:
                print("Error: No file list path provided.")
                return
            ...
            if not input_paths:
                print("Error: No valid paths found in file list.")
                return
            
            print(f"\n  ✓ Found {len(input_paths)} valid paths from file list")
            
            # Ask about recursive search for directories
            recursive_input = input("Search subdirectories recursively? (y/n) [n]: ").strip().lower()
            recursive = recursive_input == 'y'
        ...
        # Step 2: Get output directory
        print("\n" + "-"*80)
        print("OUTPUT CONFIGURATION".center(80))
        print("-"*80)
        
        output_dir = input("\nOutput directory for HDF5 datasets [current directory]: ").strip() or "."
        output_dir = Path(output_dir).expanduser().resolve()
        output_dir.mkdir(parents=True, exist_ok=True)
        
        # Step 3: Get dataset name
        # Try to derive a sensible default name
        if len(input_paths) == 1:
            path = Path(input_paths[0])
            if path.is_file():
                default_name = path.stem  # Filename without extension
            else:
                default_name = path.name  # Directory name
        else:
            default_name = "combined_dataset"
        
        dataset_name = input(f"Dataset base name [{default_name}]: ").strip() or default_name
        # Remove extensions if present
        for ext in ['.json', '.xyz', '.extxyz', '.h5']:
            if dataset_name.endswith(ext):
                dataset_name = dataset_name[:-len(ext)]
        
        # Step 4: Get train/validation split ratio
        print("\n" + "-"*80)
        print("PROCESSING OPTIONS".center(80))
        print("-"*80)
        
        train_ratio = input("\nTrain/validation split ratio [0.85]: ").strip() or "0.85"
        try:
        ...
        # Step 8: Get block size for temporal splitting
        block_size = input("Block size for temporal splitting [100]: ").strip() or "100"
        try:
        ...
        # Step 9: Get random seed
        random_seed = input("Random seed for reproducibility [42]: ").strip() or "42"
        try:
            random_seed = int(random_seed)
        except ValueError:
            print("Invalid random seed. Using default 42")
            random_seed = 42
        
        # Step 10: PBC handling
        print("\nPBC handling options:")
        print("  auto   - Include PBC if cell is present, skip otherwise")
        print("  strict - Require PBC for all configurations")
        print("  mixed  - Allow mixed PBC/non-PBC configurations")
        pbc_handling = input("PBC handling mode [auto]: ").strip().lower() or "auto"
        if pbc_handling not in ["auto", "strict", "mixed"]:
            print("Invalid option. Using 'auto'")
            pbc_handling = "auto"
        
        # Step 11: Include stress
        include_stress = input("Include stress tensor data? (y/n) [n]: ").strip().lower() == "y"
        
        # Summary before proceeding
        print("\n" + "="*80)
        print("CONFIGURATION SUMMARY".center(80))
        print("="*80)
        print(f"\n  Input paths:        {len(input_paths)} path(s)")
        ...
        print(f"  Include stress:     {include_stress}")
        ...
        try:
            ...
            output_files = create_mace_h5_dataset(
            ...
            print("="*80)
            print(f"\n  Training set:   {output_files['train']}")
            print(f"  Validation set: {output_files['valid']}")
            print("\nDatasets are ready for MACE training!")
            
        except ImportError as e:
            print(f"\n✗ Error: Could not import ml_dataset_converter module")
            print(f"  Make sure ml_dataset_converter.py is in your Python path")
            print(f"  Details: {e}")
        except Exception as e:
